...
RUN addgroup -S -g ${GROUP_ID} ${GROUP} && \
    ...
    ln -sf ${APPDIR}/nexus_iq_casc/iq-apply-conf.py /usr/local/bin/iq-apply-conf && \
    ln -sf ${APPDIR}/nexus_iq_casc/iq-healthcheck.py /usr/local/bin/iq-healthcheck && \
    ln -sf ${APPDIR}/nexus_iq_casc/iq-scrape-conf.py /usr/local/bin/iq-scrape-conf && \
